...
Jiazhi Liu1 Mingyu Li1
Tianze Xia6
Jiayi Mao5

Zhong-Zhi Li6 Duzhen Zhang6
Jiafeng Guo1,6,†
...
Cognitive Prompting [564], KAPING [48], Dynamic Assembly [311], etc.

Context
Processing (§4.2)

e.g., Mamba [1267], LongNet [220], FlashAttention [200], Ring Attention [682], YaRN [839],
Infini-attention [798], StreamingLLM [1185], Inf LLM [1184], Self-Refine [741], Reflexion [964],
StructGPT [495], GraphFormers [1230], KG Integration [1330], Long CoT [148], MLLMs [49], etc.
...
Activation Refilling [865], Context Window Management [1082], etc.
...
Agentic RAG Systems [973], Graph-Enhanced RAG [838], Modular RAG Architectures [316], etc.
...
Short-term & Long-term Memory [943], MemGPT [819], Memory-Enhanced Agents [577], etc.
...
API-Bank [621], MCP-RADAR [314], GTA benchmark [1098], PLAY2PROMPT [263], etc.
...
Coordination Strategies [631], Agent Communication Languages [360], CoA [1337], etc.
...
Brittleness Assessment [1268], Contextual Calibration [384], Multi-dimensional Feedback [288], etc.
...
NarrativeQA [556], MEMENTO [572], API-Bank [621], Mind2Web [206], SWE-Bench [500], etc.
...
O(n2) Scaling Limitations [737, 299], Transactional Integrity [128], Multi-Tool Coordination [314],
Self-Validation Dependencies [394], Context Handling Failures [214], Attribution Challenges [1122],
Safety-oriented Evaluation [87], Agent Assessment [973], Orchestration Evaluation [901], etc.

e.g., Theoretical Foundations [1141], Scaling Laws [737], O(n2) Computational Challenges [299],
Multi-modal Integration [482], Compositional Understanding [841], Context Optimization [669],
Frameworks for Multi-agent Coordination [128], Information-theoretic Analysis [314], etc.
...
Tool-Integrated Reasoning [314], Agentic Systems [973],Self-Refinement Mechanisms [741], etc.
...
Production Deployment Scalability [1236], Safety [973] and Ethical Considerations [841], etc.
...
are not arbitrary; they map directly to the core technical domains of this survey:
...
Open ResourceClose Resource202020212023202420252025.07Memory SystemsAdvanced RAGTool-Integrated ReasoningSelf-RAGDPRRAFTHippoRAGDeepSeek-R1Multi-Agent SystemsNTMMEM0MEM1ReActGorillaToolACEReToolCAMELAutoGenCrewAISagaLLMACP AgentOrchestraProAgentRecMindICX-MTGenReadGSM-ICAdaptive-RAGCRAGRAGFusionRECITERETROKGI-SlotMulti-Head RAGLightRAGCDF-RAGGraphRAGPlanRAGRag-gymModular RAGGRAGHM-RAGComposeRAGFlahRAGStructMem-LLMSCMMinervaMEMENTOGenerative AgentsConstitutional AITiMSparrowCAMELoTLarimarExplicit-Memory AgentEthical LTM AssistantsWebGPTReflexionA-MEMMemLLMRecallMGranite-Function CallingALMs SurveyTool Learning SurveyToolformerChameleonAdvancing TALMsSecure A2ASwarmIPA Assistant3S OrchestratorKQMLAgentSpotter Call-Graph ProfilingA2AFIPA ACLSmurfsCoAChatCoTAPI-BankToolLLMToRARAGMCPChatDevMetaGPTRAPTOROP-RAGOpenAI-O1StreamingRAGARISTBFCLToolPlannerGTAPlay2PromptMCP-RADARMemGPTMemoryBankREMEMBERERHuggingGPTMemorySandboxMAIDialMemOSImplementationACLANP• cinstr: System instructions and rules (Context Retrieval and Generation, Sec. 4.1).
...
is the ground-truth or ideal output. This optimization is subject to hard constraints, most notably the
Y∗
τ
...
1, . . . , Formatn), where each function must be optimized
and concatenation operations, A = Concat ◦ (Format
...
Scalability
Error Analysis Manual inspection and iterative refinement.

Brittleness increases with length and complexity. Manages complexity through modular composition.
...
demonstrating task execution through strategic example selection [1371, 405, 103, 552, 794, 1381]. In-
...
Foundational ComponentsContext Retrieval and Generation External Knowledge RetrievalDynamic Context AssemblyPrompt Engineering and Context Generation Context ProcessingLong Sequence Processing Self-Refinement and AdaptationRelational and Structured Information IntegrationContext Management Fundamental ConstraintsMemory Hierarchies and Storage ArchitecturesContext CompressionContextEngineeringLong-TermMemoryAvailableToolsUserPromptRetrievedInformationCognitive Architecture Integration Cognitive prompting implements structured human-like operations
...
Assembly Functions and Orchestration Mechanisms The assembly function A encompasses template-
...
inference tokens [876, 877]. Linear attention mechanisms reduce complexity from O(N2) to O(N) by
...
head attention while reducing memory requirements during decoding [16, 1351]. FlashAttention exploits
...
with attention computation [682]. Sparse attention techniques include Shifted sparse attention (S2-Attn) in
...
8× on 32K token sequences [1351]. StreamingLLM enables processing infinitely long sequences without
fine-tuning by retaining critical “attention sink” tokens together with recent KV cache entries, demonstrating
up to 22.2× speedup over sliding window recomputation with sequences up to 4 million tokens [1185].
...
throughput by up to 29× while reducing latency by up to 1.9× [1343].

Context compression techniques like QwenLong-CPRS implement dynamic context optimization mecha-
nisms enabling multi-granularity compression guided by natural language instructions [952]. Inf LLM stores
...
tion [1021, 588]. Context-based meta-reinforcement learning methods enhance performance through direct
...
but also inferring their combined holistic meaning [389]. Compounding these issues is our limited mecha-
...
[1391], specialized modules like ContextQFormer for conversational memory [595], and dynamic, query-
aware frame selection for video [587]. MLLMs also show emergent communication efficiency over extended
...
77.6% MRR, 0.32 BLEU improvement Query parsing and sub-graph retrieval
...
Planning-retrieval-reasoning framework N/A
...
representations [1330]. QA-GNN implements bidirectional attention mechanisms connecting question-
...
Pan et al. [823]
StructLM [1402]
Shao et al. [946] Tables, databases, KGs
...
Pre-training & inference integration Synergized LLMs + KGs
...
Schema linking & syntax prediction Text-to-SQL tasks
...
ditional operating systems [819]. These architectures consist of main context containing system instructions,
...
gisting to create concise representations, and interactive look-up for information retrieval [1211]. Compressor-
...
[164, 1362]. Knowledge-organization methods structure memory into interconnected semantic networks en-
...
dancy and improve speed [1399]. These compression techniques can be combined with other long-context
...
+Efficiency frameworks +Complex domains Deep exploration
...
resemble human conversations [891]. Task-oriented LLM systems benefit from structured context manage-
...
supplementing LLMs with access to external sources of dynamic information [216, 934]. These capabili-
...
ness [1382]. FlashRAG provides a modular toolkit with 5 core modules and 16 subcomponents enabling
...
Retrieval-Augmented GenerationModular RAG ArchitecturesAgentic RAG SystemsGraph-Enhanced RAGExternalInformationApplicationsInformation Selection Memory OptimizationKnowledge Retrieval DynamicFrameworksselect appropriate strategies [444, 445]. PlanRAG improves decision-making through plan-then-retrieve
...
in structured formats enabling efficient querying and semantic relationship capture [166, 1066]. Graph-
...
with GNNs excelling at capturing entity associations and improving knowledge consistency [232, 116]. GNN-
...
based retrieval [1359, 936, 134]. These structured approaches enable transparent reasoning pathways
...
documents while maintaining quality, particularly valuable for frequently changing content [520]. Produc-
...
databases or dedicated structures) [943]. From a persistence perspective, short-term memory includes key-
...
Memory SystemsMemory-Enhanced AgentsEvaluation and ChallengesMemory ArchitecturesUltra-longContextContext WindowSelf-AttentionExternal MemoryHierarchical MemoryImplementation-based classifications identify parametric memory (knowledge encoded in model weights),
...
to acquire and process information temporarily within context windows [1189, 103]. This enables few-
...
emotion-aware frameworks employing Mood-Dependent Memory theory [450], and memorization mecha-
...
information lists [483]. Memory retrieval operates through sequential access (retrieving content in consecu-
...
reflective feedback mechanisms [1308]. Advanced systems like MemLLM implement structured read-write
...
In conversational AI, memory systems enable more natural, human-like
...
external memory [1169]. Applications span recommendation systems (RecMind providing personalized rec-
...
including user preferences and historical interactions [444]. Domain-specific implementations include health-
care assistants employing memory coordination for medical interactions [1325, 1316], recommendation
...
tual memory and long-term external storage [38, 371, 1193, 513].

Non-parametric approaches maintain frozen LLM parameters while leveraging external resources like
RAG to enrich task contexts [942]. Systems like Reflexion implement verbal reinforcement through self-
...
memory organization and relevance-based retrieval [1211]. Commercial and open-source implementa-
...
mulating knowledge incrementally over time [1365, 1364], despite advances in working memory through
attentional tagging mechanisms enabling flexible memory representation control [870]. This limitation pre-
...
tations [288]. Dynamic memory usage in real-world applications poses evaluation challenges, as controlled
...
strating improvements while preserving general capabilities [579, 446, 366, 1271].
...
Tool-Augmented SystemsFunction Calling MechanismsTool-Integrated ReasoningEnvironment InteractionFrom "Text Generator" to "World Interactor"External ToolsDynamicDataSearchEngineUserInteractionCalculation Enginemulti-hop scenarios [621, 363, 377, 1264, 827, 995, 1257, 987].
...
cognitive processing with external resource engagement to achieve targeted outcomes [1095]. This mech-
...
actions interface with external information sources [1254]. Chameleon introduced plug-and-play composi-
...
ing speech act theory [373, 82, 663, 284]. FIPA ACL enhanced this foundation through semantic frameworks
...
Contemporary Protocol Ecosystem Contemporary standardized protocols address fragmentation chal-
lenges hindering LLM agent collaboration [1244, 1137, 412]. MCP functions as “USB-C for AI,” standardizing
...
delegation and secure collaboration via JSON-based lifecycle models [622, 250, 934]. ACP provides general-
...
Multi-Agent SystemsOrchestration MechanismsCommunication Protocols Coordination StrategiesConnection and CollaborationProgressive layering strategy: MCP provides tool access, ACP enables message exchange, A2A supports peer
...
ing agent selection, context distribution, and interaction flow control [902], enabling effective cooperation
...
ensuring task progression, and addressing task deviations [175]. Advanced orchestration frameworks incor-
...
tenance encompassing both episodic and semantic information [214, 1122], while central orchestrator
...
fundamental limitations in current evaluation frameworks and system capabilities [778, 1098, 126]. Evalua-
...
independent validation procedures to address the limitations of systems that rely exclusively on LLM self-
validation capabilities [128, 394].
...
Context scaling efficiency faces fundamental computational challenges, with current attention mecha-
...
ity and reasoning quality preservation [299, 1236, 351]. Position interpolation and extension techniques
...
standing [266, 323]. Bi et al. [75] further propose a divide-and-conquer approach to encode text-attributed
...
mance across diverse tasks [1267, 737]. Specialized position encoding methods and parameter-efficient
...
lighting fundamental limitations in current reasoning and planning capabilities [778, 1098, 126]. Advanced
...
aware adaptation and personalization mechanisms that enhance human-AI team performance [1378, 206].
...
efficient architectures and sliding attention mechanisms [299, 1236].
...
[10] Emre Can Acikgoz, Cheng Qian, Hongru Wang, Vardhan Dongre, Xiusi Chen, Heng Ji, Dilek Hakkani-
...
Introducing the model context protocol, November 2024. URL https://www.
...
[60] Azadeh Beiranvand and S. M. Vahidipour.

Integrating structural and semantic signals in text-
...
[115] He Cao, Zhenwei An, Jiazhan Feng, Kun Xu, Liwei Chen, and Dongyan Zhao. A step closer to com-
...
preprint arXiv:2410.05695, 2024. URL https://arxiv.org/abs/2410.05695.

[148] Qiguang Chen, Libo Qin, Jinhao Liu, Dengyun Peng, Jiannan Guan, Peng Wang, Mengkang Hu,
Yuhang Zhou, Te Gao, and Wangxiang Che. Towards reasoning era: A survey of long chain-
...
thought for reasoning large language models, arXiv preprint arXiv:2503.09567, 2025. URL https:
...
[152] S Chen, Y Wang, YF Wu,

Advancing tool-augmented large
2024.
...
[183] Emile Contal and Garrin McGoldrick. Ragsys: Item-cold-start recommender as rag system. IR-

RAG@SIGIR, 2024.

[184] Erica Coppolillo.
...
[188] crewAI Inc. crewai: Framework for orchestrating role-playing, autonomous ai agents. https:

//github.com/crewAIInc/crewAI, 2024. [Online; accessed 17-July-2025].

[189] A. Cruz, André V. dos Santos, R. Santiago, and B. Bedregal. A fuzzy semantic for bdi logic. Fuzzy
...
[193] Yue Cui, Liuyi Yao, Shuchang Tao, Weijie Shi, Yaliang Li, Bolin Ding, and Xiaofang Zhou. En-
...
[194] C. Curto, A. Degeratu, and V. Itskov. Flexible memory networks. Bulletin of Mathematical Biology,
...
[199] Tri Dao. Flashattention-2: Faster attention with better parallelism and work partitioning. International
...
[207] Yang Deng, Wenqiang Lei, Hongru Wang, and Tat seng Chua. Prompting and evaluating large lan-
...
[210] Brouillet Denis and Versace Rémy. The nature of the traces and the dynamics of memory. Psychology
...
[228] Kaiwen Dong. Large language model applied in multi-agent systema survey. Applied and Computa-
...
[237] Mauro Dragone. Component & service-based agent systems: Self-osgi. International Conference on
...
[250] Abul Ehtesham, Aditi Singh, Gaurav Kumar Gupta, and Saket Kumar. A survey of agent interoperabil-
...
[254] Kazem Faghih, Wenxiao Wang, Yize Cheng, Siddhant Bharti, Gaurang Sriramanan, S. Balasub-
...
[258] Yue Fan, Xiaojian Ma, Rujie Wu, Yuntao Du, Jiaqi Li, Zhi Gao, and Qing Li. Videoagent: A memory-
...
[279] Chrisantha Fernando, Dylan Banarse, H. Michalewski, Simon Osindero, and Tim Rocktäschel. Prompt-
...
[282] M. Ferrag, Norbert Tihanyi, and M. Debbah. Reasoning beyond limits: Advances and open prob-
...
[286] Paolo Finotelli and Francis Eustache. Mathematical modeling of human memory. Frontiers in
...
[290] Zafeirios Fountas, Martin A Benfeghoul, Adnan Oomerjee, Fenia Christopoulou, Gerasimos Lam-
...
[295] Honghao Fu, Yilang Shen, Yuxuan Liu, Jingzhong Li, and Xiang Zhang. Sgcn: a multi-order neigh-
...
[301] Tiantian Gan and Qiyao Sun. Rag-mcp: Mitigating prompt bloat in llm tool selection via retrieval-
...
[303] Anish Ganguli, Prabal Deb, and Debleena Banerjee. Mark: Memory augmented refinement of knowl-
...
[310] Shuzheng Gao, Xinjie Wen, Cuiyun Gao, Wenxuan Wang, and Michael R. Lyu. What makes good in-
...
[312] Weiguo Gao. Mep: Multiple kernel learning enhancing relative positional encoding length extrapola-
...
[321] Tao Ge, Jing Hu, Xun Wang, Si-Qing Chen, and Furu Wei.
...
[327] Antonios Georgiou, M. Katkov, and M. Tsodyks. Retroactive interference model of forgetting. Journal
...
[333] Abdellah Ghassel, Ian Robinson, Gabriel Tanase, Hal Cooper, Bryan Thompson, Zhen Han, V. Ioanni-
...
[348] Sven Gronauer and K. Diepold. Multi-agent deep reinforcement learning: a survey. Artificial
...
[362] Yuan Guo, Tingjia Miao, Zheng Wu, Pengzhou Cheng, Ming Zhou, and Zhuosheng Zhang. Atomic-
...
[364] Zirui Guo, Lianghao Xia, Yanhua Yu, Tu Ao, and Chao Huang. Lightrag: Simple and fast retrieval-
...
[370] Bernal Jimenez Gutierrez, Yiheng Shu, Yu Gu, Michihiro Yasunaga, and Yu Su. Hipporag: Neuro-
biologically inspired long-term memory for large language models. Neural Information Processing
Systems, 2024.

[371] Kelvin Guu, Kenton Lee, Zora Tung, Panupong Pasupat, and Ming-Wei Chang. Realm: Retrieval-
...
[379] Tingxu Han, Zhenting Wang, Chunrong Fang, Shiyun Zhao, Shiqing Ma, and Zhenyu Chen. Token-
...
[384] Mareike Hartmann and Alexander Koller. A survey on complex tasks for goal-directed interac-
tive agents, arXiv preprint arXiv:2409.18538, 2024. URL https://arxiv.org/abs/2409.
18538v1.
...
[392] Shengtao He. Achieving tool calling functionality in llms using only prompt engineering with-
out fine-tuning, arXiv preprint arXiv:2407.04997, 2024. URL https://arxiv.org/abs/2407.
04997v1.

[393] Wenchong He, Liqian Peng, Zhe Jiang, and Alex Go. You only fine-tune once: Many-shot in-
context fine-tuning for large language model, arXiv preprint arXiv:2506.11103, 2025. URL https:
//arxiv.org/abs/2506.11103v1.

[394] Xu He, Di Wu, Yan Zhai, and Kun Sun. Sentinelagent: Graph-based anomaly detection in multi-
agent systems, arXiv preprint arXiv:2505.24201, 2025. URL https://arxiv.org/abs/2505.
24201v1.

[395] Yang He, Xiao Ding, Bibo Cai, Yufei Zhang, Kai Xiong, Zhouhao Sun, Bing Qin, and Ting Liu. Self-
...
[398] James B. Heald, M. Lengyel, and D. Wolpert. Contextual inference in learning and memory. Trends
...
[404] Thomas F. Heston and Charya Khun. Prompt engineering in medical education. International Medical

Education, 2023.

[405] Dollaya Hirunyasiri, Danielle R. Thomas, Jionghao Lin, K. Koedinger, and Vincent Aleven. Com-
...
[416] Yubin Hong, Chaofan Li, Jingyi Zhang, and Yingxia Shao. Fg-rag: Enhancing query-focused summa-

rization with context-aware fine-grained graph rag. arXiv preprint, 2025.

[417] Thanapapas Horsuwan, Piyawat Lertvittayakumjorn, Kasidis Kanwatchara, B. Kijsirikul, and P. Va-
teekul. Meta lifelong-learning with selective and task-aware adaptation. IEEE Access, 2024.

[418] A. N. Hoskin, A. Bornstein, K. Norman, and J. Cohen. Refresh my memory: Episodic memory rein-
...
exploration in unknown environments via deep reinforcement learning.
...
task automation, arXiv preprint arXiv:2505.23885, 2025. URL https://arxiv.org/abs/2505.
23885.
...
[439] Ting Hu, Christoph Meinel, and Haojin Yang. Scaled prompt-tuning for few-shot natural lan-
...
[446] Chengrui Huang, Shen Gao, Zhengliang Shi, Dongsheng Wang, and Shuo Shang. Ttpa: Token-
...
[454] Sirui Huang, Hanqian Li, Yanggan Gu, Xuming Hu, Qing Li, and Guandong Xu. Hyperg: Hypergraph-
...
[462] Ziheng Huang, Sebastian Gutierrez, Hemanth Kamana, and Stephen MacNeil. Memory sand-
...
[466] Thorsten Händler. Balancing autonomy and alignment: A multi-dimensional taxonomy for au-
...
[468] IBM. What is agent communication protocol (acp)? https://www.ibm.com/think/topics/
...
[472] Yoichi Ishibashi, Taro Yano, and M. Oyamada. Can large language models invent algorithms to im-
...
[478] Tejas Jade and Alex Yartsev. Chatgpt for automated grading of short answer questions in mechani-
...
[481] H. Jaleel, Jane J. Stephan, and Sinan Naji. Multi-agent systems: A review study. Ibn AL- Haitham
...
[487] Gang Ji and J. Bilmes. Multi-speaker language modeling. North American Chapter of the Association
...
[491] Caigao Jiang, Siqiao Xue, James Zhang, Lingyue Liu, Zhibo Zhu, and Hongyan Hao. Learning large-
...
[499] Zhuoxuan Jiang, Haoyuan Peng, Shanshan Feng, Fan Li, and Dongsheng Li. Llms can find mathemat-
...
[504] Haolin Jin, Linghan Huang, Haipeng Cai, Jun Yan, Bo Li, and Huaming Chen. From llms to llm-
...
[509] Weiqiang Jin, Hongyang Du, Biao Zhao, Xingwu Tian, Bohang Shi, and Guang Yang. A compre-
...
[513] Jeff Johnson, Matthijs Douze, and H. Jégou. Billion-scale similarity search with gpus. IEEE Transac-

tions on Big Data, 2017.

[514] Jeff A. Johnson and Daniel H. Bullock. Fragility in ais using artificial neural networks. Communications
...
[516] Kurmanbek Kaiyrbekov, Nic Dobbins, and Sean D. Mooney. Automated survey collection with llm-
...
[531] T. Kelley, R. Thomson, and Jonathan Milton. Standard model of mind: Episodic memory. Biologically
...
[542] Geunwoo Kim, P. Baldi, and S. McAleer. Language models can solve computer tasks. Neural
...
[546] Jiin Kim, Byeongjun Shin, Jinha Chung, and Minsoo Rhu. The cost of dynamic reasoning: De-
...
[549] Lukas Kirchdorfer, Robert Blümel, T. Kampik, Han van der Aa, and Heiner Stuckenschmidt. Dis-
...
[554] Nikita Kitaev, Lukasz Kaiser, and Anselm Levskaya. Reformer: The efficient transformer. International
...
[558] Jing Yu Koh, Robert Lo, Lawrence Jang, Vikram Duvvur, Ming Chong Lim, Po-Yu Huang, Gra-
...
[565] K. Kravari and Nick Bassiliades. A survey of agent platforms. Journal of Artificial Societies and Social
...
language models for local life services, arXiv preprint arXiv:2506.02720, 2025. URL https:
//arxiv.org/abs/2506.02720v1.

[577] LangChain Team. Memory in langgraph. https://langchain-ai.github.io/langgraph/
...
[579] B. Lattimer, Varun Gangal, Ryan McDonald, and Yi Yang. Sparse rewards can self-train dia-
...
[581] Hung Le, T. Tran, and S. Venkatesh. Self-attentive associative memory. International Conference on
...
llms as chatbot modules for long open-domain conversation.
...
[589] Namkyeong Lee, E. Brouwer, Ehsan Hajiramezanali, Chanyoung Park, and Gabriele Scalia. Rag-
...
[593] Yu-Ting Lee, Hui-Ying Shih, Fu-Chieh Chang, and Pei-Yuan Wu. An explanation of intrinsic self-
...
[602] Chuanhao Li, Runhan Yang, Tiankai Li, Milad Bafarassat, Kourosh Sharifi, Dirk Bergemann, and Zhuo-
...
[612] Jiarui Li, Ye Yuan, and Zehua Zhang. Enhancing llm factual accuracy with rag to counter hal-
...
[627] X Li, H Zou, and P Liu. Torl: Scaling tool-integrated rl. 2025. URL https://arxiv.org/abs/
...
[635] Yuan Li, Yixuan Zhang, and Lichao Sun. Metaagents: Simulating interactions of human behav-
...
[663] David Lillis. Internalising interaction protocols as first-class programming elements in multi agent sys-
...
Zhang, Lanbo Li, Xiafei Qiu, Shen Li, Zhigang Ji, Tao Xie, Yong Li, and Wei Lin.
Infinite-llm:
...
[674] Tal Linzen, Emmanuel Dupoux, and Yoav Goldberg. Assessing the ability of lstms to learn syntax-
...
[682] Hao Liu, Matei Zaharia, and Pieter Abbeel. Ring attention with blockwise transformers for near-

infinite context. International Conference on Learning Representations, 2023.

[683] Haotian Liu, Chunyuan Li, Qingyang Wu, and Yong Jae Lee. Visual instruction tuning. Neural

Information Processing Systems, 2023.

[684] Jie Liu, Pan Zhou, Yingjun Du, Ah-Hwee Tan, Cees G. M. Snoek, J. Sonke, and E. Gavves. Capo: Co-
...
[686] Junwei Liu, Kaixin Wang, Yixuan Chen, Xin Peng, Zhenpeng Chen, Lingming Zhang, and Yil-
...
[692] Pei Liu, Xin Liu, Ruoyu Yao, Junming Liu, Siyuan Meng, Ding Wang, and Jun Ma. Hm-rag: Hierar-
...
[708] Leo S. Lo. The art and science of prompt engineering: A new literacy in the information age. Internet
...
institutions, arXiv preprint arXiv:2503.13524, 2025. URL https://arxiv.org/abs/2503.
13524v1.
...
[718] Yinquan Lu, H. Lu, Guirong Fu, and Qun Liu. Kelm: Knowledge enhanced pre-trained language repre-
...
finetuning with knowledgeable adaptation.
...
[732] Jianxiang Ma. Research on the role of llm in multi-agent systems: A survey. Applied and Computational
...
[736] Xinbei Ma, Yeyun Gong, Pengcheng He, Hai Zhao, and Nan Duan. Query rewriting for retrieval-
...
[747] Qiheng Mao, Zemin Liu, Chenghao Liu, Zhuo Li, and Jianling Sun. Advancing graph representa-
...
[749] Sophia Maria. Compass-v2 technical report, arXiv preprint arXiv:2504.15527, 2025. URL https:

//arxiv.org/abs/2504.15527v1.

[750] Viorica Marian and U. Neisser. Language-dependent recall of autobiographical memories. Journal of

experimental psychology. General, 2000.

[751] S. Mariani and Andrea Omicini. Special issue “multi-agent systems”: Editorial. Applied Sciences,
...
[757] Costas Mavromatis, V. Ioannidis, Shen Wang, Da Zheng, Soji Adeshina, Jun Ma, Han Zhao, C. Falout-
...
heuristics in natural language inference. Annual Meeting of the Association for Computational
Linguistics, 2019.
...
[768] T. Meiser and A. Bröder. Memory for multidimensional source information. Journal of Experimental
...
[783] Ishan Misra and L. Maaten. Self-supervised learning of pretext-invariant representations. Computer
...
[792] Shinsuke Mori. A stochastic parser based on an slm with arboreal context trees.
...
[797] Tergel Munkhbat, Namgyu Ho, Seohyun Kim, Yongjin Yang, Yujin Kim, and Se young Yun. Self-
...
[803] Sundaraparipurnan Narayanan and Sandeep Vishwakarma. Guard-d-llm: An llm-based risk assess-
...
[810] C. Nourani and P. Eklund. Concepts ontology algebras and role descriptions. Conference on Computer
...
[817] Gustavo Ortiz-Hernández, Alejandro Guerra-Hernández, J. Hübner, and W. A. Luna-Ramírez. Mod-
...
[828] A Parisi, Y Zhao, and N Fiedel. Talm: Tool augmented language models. 2022. URL https:
...
[850] Moritz Plenz and Anette Frank. Graph language models. Annual Meeting of the Association for

Computational Linguistics, 2024.

[851] Sean M. Polyn, K. Norman, and M. Kahana. A context maintenance and retrieval model of organiza-

tional processes in free recall. Psychology Review, 2009.

[852] Liam Pond and Ichiro Fujinaga. Teaching llms music theory with in-context learning and chain-
of-thought prompting: Pedagogical strategies for machines. International Conference on Computer
Supported Education, 2025.

122

[853] V Porcu. The role of memory in llms: Persistent context for smarter conversations. Int. J. Sci. Res.
...
memory, arXiv preprint arXiv:2505.22006, 2025. URL https://arxiv.org/abs/2505.
22006v1.
...
understanding, arXiv preprint arXiv:2406.10505, 2024. URL https://arxiv.org/abs/2406.
10505.
...
[886] Ella Rabinovich and Ateret Anaby-Tavor. On the robustness of agentic function calling. Proceedings
...
[888] Zackary Rackauckas. Rag-fusion: a new take on retrieval-augmented generation. International
...
schemas: Dynamic tree memory representation for llms.
Representations, 2024.
...
[910] Hayley Ross, A. Mahabaleshwarkar, and Yoshi Suhara. When2call: When (not) to call tools. North
...
[920] Iman Saberi and Fatemeh Fard. Context-augmented code generation using programming knowl-
edge graphs, arXiv preprint arXiv:2410.18251, 2024. URL https://arxiv.org/abs/2410.
18251v2.

[921] Abdulfattah Safa and Gözde Gül Sahin. A zero-shot open-vocabulary pipeline for dialogue under-
...
[925] Rana Salama, Jason Cai, Michelle Yuan, Anna Currey, Monica Sunkara, Yi Zhang, and Yas-
...
[929] A. Samsonovich. Toward a unified catalog of implemented cognitive architectures. Biologically
...
[932] G. Santos, Rita Maria Silva Julia, and Marcelo Zanchetta do Nascimento. Diverse prompts: Illumi-
...
[954] Zhuocheng Shen. Llm with tools: A survey, arXiv preprint arXiv:2409.18807, 2024. URL https:
...
Kyunghyun Cho, Gichang Lee, W. Park, Jung-Woo Ha, and Nako Sung. On the effect of pre-
...
[967] Connor Shorten, T. Khoshgoftaar, and B. Furht. Text data augmentation for deep learning. Journal
...
[972] Anmolika Singh and Yuhang Diao. Leveraging large language models for optimized item categoriza-
...
[977] Sanchit Sinha, Yuguang Yue, Victor Soto, Mayank Kulkarni, Jianhua Lu, and Aidong Zhang. Maml-
en-llm: Model agnostic meta-training of llms for improved in-context learning. Knowledge Discovery
and Data Mining, 2024.

[978] Colin Sisate, Alistair Goldfinch, Vincent Waterstone, Sebastian Kingsley, and Mariana Black-
...
[983] Chan Hee Song, Jiaman Wu, Clay Washington, Brian M. Sadler, Wei-Lun Chao, and Yu Su. Llm-
planner: Few-shot grounded planning for embodied agents with large language models.
IEEE
...
[989] S. Srinivasa and Jayati Deshmukh. Paradigms of computational agency. Novel Approaches to Informa-
...
[999] Jinyan Su, Jennifer Healey, Preslav Nakov, and Claire Cardie. Between underthinking and overthink-
...
[1010] Lei Sun, Xinchen Wang, and Youdi Li. Pyramid-driven alignment: Pyramid principle guided integra-
...
[1011] Liangtai Sun, Xingyu Chen, Lu Chen, Tianle Dai, Zichen Zhu, and Kai Yu. Meta-gui: Towards multi-
...
[1015] Rao Surapaneni, Miku Jha, Michael Vakoc,
protocol
...
[1036] Xuemei Tang, Jun Wang, and Q. Su. Chinese word segmentation with heterogeneous graph neural net-
work, arXiv preprint arXiv:2201.08975, 2022. URL https://arxiv.org/abs/2201.08975v1.

[1037] Yiqing Tang, Xingyuan Dai, Chengchong Zhao, Qi Cheng, and Yisheng Lv. Large language model-
...
[1039] Yunlong Tang, Daiki Shimada, Jing Bi, Hang Hua, and Chenliang Xu. Empowering llms with pseudo-
...
[1041] Denis Tarasov and Kumar Shridhar. Distilling llms’ decomposition abilities into compact language mod-
...
[1068] J. D. Velásquez-Henao, Carlos Jaime Franco-Cardona, and Lorena Cadavid-Higuita. Prompt engineer-
ing: a methodology for optimizing interactions with ai-language models in the field of engineering.
DYNA, 2023.

[1069] Gaurav Verma, Rachneet Kaur, Nishan Srishankar, Zhen Zeng, T. Balch, and Manuela Veloso. Adap-
...
[1076] D. Walton. Using argumentation schemes to find motives and intentions of a rational agent. Argument
...
[1078] Jun Wan and Lingrui Mei. Large language models as computable approximations to solomonoff in-
...
[1080] Bernie Wang, Si ting Xu, K. Keutzer, Yang Gao, and Bichen Wu.
...
[1100] Lei Wang, Wanyu Xu, Yihuai Lan, Zhiqiang Hu, Yunshi Lan, Roy Ka-Wei Lee, and Ee-Peng Lim. Plan-
...
[1117] Xiaohan Wang, Yuhui Zhang, Orr Zohar, and S. Yeung-Levy. Videoagent: Long-form video under-
...
Yawen Wang, Kaiyu Huang, Yile Wang, Peng Li, and Yang Liu. Mucar: Benchmarking multi-
...
[1127] Yaqi Wang and Haipei Xu. Srsa: A cost-efficient strategy-router search agent for real-world human-
...
[1133] Yu Wang, Dmitry Krotov, Yuanzhe Hu, Yifan Gao, Wangchunshu Zhou, Julian McAuley, Dan Gut-
...
[1166] Panlong Wu, Ting Wang, Yifei Zhong, Haoqi Zhang, Zitong Wang, and Fangxin Wang. Deep-
...
[1177] Zengqing Wu and Takayuki Ito. The hidden strength of disagreement: Unraveling the consensus-
...
[1181] Yuchen Xia, Manthan Shenoy, N. Jazdi, and M. Weyrich. Towards autonomous system: flexible mod-
...
[1188] Yilin Xiao, Chuang Zhou, Qinggang Zhang, Bo Li, Qing Li, and Xiao Huang. Reliable reason-
...
[1191] Yue Xing, Tao Yang, Yijiashun Qi, Minggu Wei, Yu Cheng, and Honghui Xin. Structured memory mech-
...
[1194] Junjie Xiong, Changjia Zhu, Shuhang Lin, Chong Zhang, Yongfeng Zhang, Yao Liu, and Lingyao Li. In-
...
[1199] Chunmei Xu, Shengheng Liu, Cheng Zhang, Yongming Huang, Zhaohua Lu, and Luxi Yang. Multi-
...
[1204] Minrui Xu, Hongyang Du, Dusist Niyato, Jiawen Kang, Zehui Xiong, Shiwen Mao, Zhu Han, A. Ja-
...
[1213] Yifei Xu, Jingqiao Zhang, Ru He, Liangzhu Ge, Chao Yang, Cheng Yang, and Ying Wu. Sas: Self-
...
[1226] Jianxin Yang. Longqlora: Efficient and effective method to extend context length of large lan-
...
[1228] Jingkang Yang, Yuhao Dong, Shuai Liu, Bo Li, Ziyue Wang, Chencheng Jiang, Haoran Tan, Ji-
...
[1238] Wang Yang, Zirui Liu, Hongye Jin, Qingyu Yin, Vipin Chaudhary, and Xiaotian Han. Longer con-
...
[1257] Shunyu Yao, Noah Shinn, P. Razavi, and Karthik Narasimhan. τ-bench: A benchmark for tool-agent-
...
[1269] Peiling Yi and Yuhan Xia. Irony detection, reasoning and understanding in zero-shot learning. IEEE
...
[1303] B Zhang, K Zhou, X Wei, and X Zhao. . . . Evaluating and improving tool-augmented computation-
...
[1310] Duzhen Zhang, Yahan Yu, Jiahua Dong, Chenxing Li, Dan Su, Chenhui Chu, and Dong Yu. Mm-
In Findings of the Association for
...
[1313] Hengyu Zhang. Sinklora: Enhanced efficiency and chat capabilities for long-context large lan-
...
[1316] Kai Zhang, Fubang Zhao, Yangyang Kang, and Xiaozhong Liu. Llm-based medical assistant personal-
...
[1328] Wentao Zhang, Ce Cui, Yilei Zhao, Rui Hu, Yang Liu, Yahui Zhou, and Bo An. Agentorchestra: A hi-
...
verification and wrong information, arXiv preprint arXiv:2410.04463, 2024. URL https://arxiv.
org/abs/2410.04463.
...
[1355] Shangziqi Zhao, Jiahao Yuan, Guisong Yang, and Usman Naseem. Can pruning improve reason-
...
[1359] Yibo Zhao, Jiapeng Zhu, Ye Guo, Kangkang He, and Xiang Li. E2graphrag: Streamlining graph-based
...
[1375] Bin Zhou, Xingwang Shen, Yuqian Lu, Xinyu Li, B. Hua, Tianyuan Liu, and Jinsong Bao. Semantic-
...
question answering.
In In Proceedings of the 2024 Conference on Empirical Methods in Natural
